...
public class Featured_adapter extends RecyclerView.Adapter<Featured_adapter.ProductViewHolder> {
    ImageLoader imageLoader = MyApplication.getInstance().getImageLoader();
    private Context mCtx;
    private List<featured_model> featured_models;
    ReadPref readPref;
    String DateToStr;
    public Featured_adapter(Context mCtx, List<featured_model> productList) {
        this.mCtx = mCtx;
        this.featured_models = productList;
    }

    @Override
    public Featured_adapter.ProductViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(mCtx);
        readPref = new ReadPref(mCtx);
        Date curDate = new Date();
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        DateToStr = format.format(curDate);
        View view = inflater.inflate(R.layout.featured_item, null);
        return new Featured_adapter.ProductViewHolder(view);
    }

    @Override
    public void onBindViewHolder(Featured_adapter.ProductViewHolder holder, int position) {
        imageLoader = MyApplication.getInstance().getImageLoader();
       final featured_model product = featured_models.get(position);
        ...
        try {
           ...
           /* if(diffDays>0.0){
                holder.day_left_text.setVisibility(View.VISIBLE);
                holder.inactive_text.setVisibility(View.GONE);
                holder.active_text.setVisibility(View.VISIBLE);
                holder.dayleft_text.setText(diffDays+" days");
                ...
                holder.day_left_text.setVisibility(View.GONE);
                holder.active_text.setVisibility(View.GONE);
                holder.inactive_text.setVisibility(View.VISIBLE);
                holder.dayleft_text.setText("Expired");
                holder.dayleft_text.setTextColor(mCtx.getResources().getColor(R.color.red));
            }*/
